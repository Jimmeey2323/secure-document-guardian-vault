...
// Add safe event handling to prevent the recordClickType error
export function safeOnClick(handler: (event: Event) => void) {
  return (event: Event) => {
    try {
      console.log('Safe onClick handler called', event);
      handler(event);
    } catch (error) {
